  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        ...
        const youtubeTrailers = trailersData.results.filter(trailer => trailer.site === 'YouTube');
    ...
    <div>
      {movieInfo && (
        <div>
          ...
          <p>Rating: {movieInfo.vote_average} ({movieInfo.vote_count} votes)</p>
...
<ul>
            {productionCompanies.map(company => (
              <li key={company.id}>{company.name} <img src={`https://image.tmdb.org/t/p/w200/${company.logo_path}`} alt={company.name}  /></li>
            ))}
          </ul>
