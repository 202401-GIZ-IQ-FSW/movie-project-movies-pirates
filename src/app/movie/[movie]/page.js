      ...
      {movieInfo && (
        <div>
          <div className="grid gap-4 grid-cols-2">
            ...
            <div className="relative right-36 top-4">
              ...
              <div className="grid grid-cols-3">
                <p className="relative top-16 text-xs">{movieInfo.runtime} min</p>
                <p className="relative top-16 right-44 text-xs">{movieInfo.release_date}</p>
                <p className="relative top-16 right-80 text-xs">lang: {movieInfo.original_language}</p>
              </div>
              <hr className="border-t-3 border-white w-1/3 relative top-16 mt-2"></hr>
              <p className="relative top-20 text-xs">
                {Array.from({ length: Math.round(movieInfo.vote_average) }, (_, index) => (
                  <span key={index}>⭐</span>
                ))}
          ...
          <div className="relative left-96 bottom-96 ml-56">
            {!showVideo ? (
              <button onClick={() => setShowVideo(true)} className="h-12 w-64 bg-purple-600 text-white 
